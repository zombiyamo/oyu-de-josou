...
- **葉のグレイン**: `GRASS_TEX`(施主撮影の草写真から抽出したグレースケール高周波
  ...
  (動的経路=注水中・枯れ進行中・崩れ中に含めると、全草を動的状態にした実測で
  フレーム時間が約1.5倍化したため意図的に除外。動的区間は「生えたて」「崩れ中」は
  数秒だが、「枯れ進行中」は種の `witherDur`(15〜60秒)ぶん質感が外れうる=
  実は無視できない長さ。それでも健康な草・枯れ草(=生涯の大半)は静的レイヤーで
  質感が乗るので、体感の劣化は小さいという判断)
