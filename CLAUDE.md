...
- **ホームセンター(自動化)**: `SHOP_ITEMS`。除草ポイント = `totalKilled - spentPoints`
  ...
  - `gardenStones` / `ponds` = **苔石・池**(クリア後の庭づくり。`inDecoration()` の
    範囲には草が生えない=買うほど雑草エリアが狭まる。形はseedで固定し保存される。
    ヤギは池に入らない。池の錦鯉は熱湯と無関係なのでOK)
  - 設置は `autoSpot()` が草の多い場所を選ぶ(飾りは `decoSpot()` で空きを選ぶ)。
    保存キーは `oyu_autoItems`
